...
export class InputHandler {
  constructor(canvas) {
    ...
    // Last touch position (for UI button detection)
    this._lastTouchX = 0;
    this._lastTouchY = 0;
    this._uiButtonHit = false; // Set by game when a UI button is tapped
  ...
  /** Get last touch X position (canvas-relative). */
  get lastTouchX() { return this._lastTouchX; }
  
  /** Get last touch Y position (canvas-relative). */
  get lastTouchY() { return this._lastTouchY; }
  
  /** Mark that a UI button was hit (prevents jump/start from firing). */
  setUIButtonHit() { this._uiButtonHit = true; }
  ...
  _onTouchEnd(e) {
    ...
    if (duration < this._tapDuration && e.changedTouches.length > 0) {
      ...
      // Store touch position for UI button detection
      const rect = this._canvas.getBoundingClientRect();
      this._lastTouchX = touch.clientX - rect.left;
      this._lastTouchY = touch.clientY - rect.top;
      
      if (displacement < this._tapThreshold) {
        // Quick tap = jump/start
        // The game will handle UI button detection separately
        this._jumpPressed = true;
        this._startPressed = true;
      }
    }
